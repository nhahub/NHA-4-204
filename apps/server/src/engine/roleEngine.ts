...
async function evaluateUserForRoleInternal({
    ...
    const skillIdByName = new Map(requiredSkillMetadata.map((skill) => [skill.name, skill.id]));
    ...
    const finalScore = clamp(totalScore + bonusScore, 0, 100);
    ...
    const insertedReports = await db
        ...
        .values({
            ...
            roleGithubScore: roleGithubScore.toString(),
    ...
    if (gapRows.length > 0) {
        await db.insert(skillGapResults).values(gapRows);
    }
...
function calculateRoleGithubScore({
    ...
    const projectAlignmentWeighted = requiredSkills.reduce((sum, skill) => {
        ...
        const coverage = clamp(projectsWithSkill / projectCount, 0, 1);
...
function getSkillImportance(skill: { weight: string } & Record<string, unknown>) {
    const importanceValue =
        typeof skill.importance === "string" || typeof skill.importance === "number"
            ? Number(skill.importance)
            : Number(skill.weight);
