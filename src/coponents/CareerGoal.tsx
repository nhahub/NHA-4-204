...
// 1. تعريف واجهة البيانات الخاصة بالهدف/الوظيفة (Goal Interface)
interface Goal {
  id: string | number;
  title: string;
  description?: string;
...
const CareerGoal = () => {
  ...
  const handleContinue = () => {
    ...
    setUserRole.mutate(
      {
        roleId: selectedGoal.id as string, // تحويل النوع ليتوافق مع الـ API المتوقع إذا لزم الأمر
      },
      {
        onSuccess: () => {
  ...
  // 2. تعيين أنواع الأيقونات بدقة باستخدام LucideIcon
